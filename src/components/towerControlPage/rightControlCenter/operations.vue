            ...
            <t-row :gutter="5">
                ...
                <t-col>
                    <t-select size="small" placeholder="区域" style="width: 80px;" v-model="selectedArea"
                        @change="onAreaChange">
                        <t-option v-for="option in towerEfpsAreaOptions" :key="option.value" :value="option.value"
                            :label="option.label">
                            {{ option.label }}
                        </t-option>
                    </t-select>
            ...
            <t-col>
                ...
                <t-dialog v-model:visible="transferVisible" header='确认移交？' :on-confirm="handleTransfer" theme="info">
                    ...
                    <el-scrollbar hight="50px">
                        <towerEfps BackgroundColor="lightskyblue" :efps-data="processingData[0] || {}" />
                    </el-scrollbar>
                </t-dialog>
            </t-col>
...
import {
    ...
    , riseHight, declineHight, setConflictFlag, setDiversion, setReturnflight, setVIP, setCreateTime, flyCommand, sendFlyCommand, transferEfps
} from '@/stores/towerEfps-store';
import { ref } from 'vue';
import towerEfps from '../towerEfps.vue'
import { towerEfpsAreaOptions } from '@/types/towerEfpsTypes';
