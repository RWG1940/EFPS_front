<template>
    <userEdit :visible="visible" header="转单面板" @update:visible="handleTransferVisibleChange">
        <template #main>
            ...
            <!-- 并行遍历三组数据 -->
            <div v-for="index in Math.max(flightPlanTransferDataGroup.length, flightInfoDataGroup.length, efpsDataGroup.length)" :key="index">
                <p style="display: flex;justify-content: center;align-items: center;font-style: italic;">---------- 数据{{ index }} ----------</p>
                <!-- 显示 flightPlanTransferData -->
                <t-row>
                    <el-tag>{{ flightPlanTransferDataGroup[index-1]?.arrivalAirport == airport ? '进港进程单' : '离港进程单' }}</el-tag>
                </t-row>
                ...
                <t-row>
                    <t-card>
                        ...
                        <t-row>
                            <t-col :span="6">计划起飞时间：{{ flightInfoDataGroup[index-1]?.scheduledDepartureTime }} </t-col>
...
const transferButton = () => {
    ...
    flightPlanTransferDataGroup.value.forEach((flightPlan, index) => {
        ...
        const currentFlightInfoData = flightInfoDataGroup.value[index]; // 获取对应的 flightInfoData
        ...
        if (flightPlan.arrivalAirport == airport.value) {
            ...
            flightParkingStandStore.addData({
                ...
                flightId: currentFlightInfoData.flightNumber
            });
        ...
        // 添加航班信息
        flightInfoStore.addData(currentFlightInfoData);
    });
